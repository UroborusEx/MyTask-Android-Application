...
public class ColorUtils {
    ...
    private static int compositeAlpha(int foregroundAlpha, int backgroundAlpha) {
        return 255 - (((255 - backgroundAlpha) * (255 - foregroundAlpha)) / Utils.FULL_ALPHA);
    }
    ...
    private static int compositeComponent(int fgC, int fgA, int bgC, int bgA, int a) {
        ...
        return (((fgC * Utils.FULL_ALPHA) * fgA) + ((bgC * bgA) * (255 - fgA))) / (a * Utils.FULL_ALPHA);
    }
    ...
    public static int calculateMinimumAlpha(@ColorInt int foreground, @ColorInt int background, float minContrastRatio) {
        if (Color.alpha(background) != Utils.FULL_ALPHA) {
            throw new IllegalArgumentException("background can not be translucent: #" + Integer.toHexString(background));
        } else if (calculateContrast(setAlphaComponent(foreground, Utils.FULL_ALPHA), background) < ((double) minContrastRatio)) {
            return -1;
        } else {
            int minAlpha = 0;
            int maxAlpha = Utils.FULL_ALPHA;
    ...
    public static void RGBToHSL(@IntRange(from = 0, to = 255) int r, @IntRange(from = 0, to = 255) int g, @IntRange(from = 0, to = 255) int b, @NonNull float[] hsl) {
        ...
        hsl[MIN_ALPHA_SEARCH_PRECISION] = constrain(s, 0.0f, 1.0f);
    ...
    public static int HSLToColor(@NonNull float[] hsl) {
        ...
        return Color.rgb(constrain(r, 0, (int) Utils.FULL_ALPHA), constrain(g, 0, (int) Utils.FULL_ALPHA), constrain(b, 0, (int) Utils.FULL_ALPHA));
    }

    @ColorInt
    public static int setAlphaComponent(@ColorInt int color, @IntRange(from = 0, to = 255) int alpha) {
        if (alpha >= 0 && alpha <= Utils.FULL_ALPHA) {
