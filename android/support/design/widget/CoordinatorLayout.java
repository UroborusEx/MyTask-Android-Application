...
public class CoordinatorLayout extends ViewGroup implements NestedScrollingParent {
    ...
    class C00021 implements Comparator<View> {
        C00021() {
        }
        ...
        public int compare(View lhs, View rhs) {
            ...
            return ((LayoutParams) rhs.getLayoutParams()).dependsOn(CoordinatorLayout.this, rhs, lhs) ? -1 : CoordinatorLayout.TYPE_ON_INTERCEPT;
        }
    }
    ...
    public static class LayoutParams extends MarginLayoutParams {
        ...
        LayoutParams(Context context, AttributeSet attrs) {
            ...
            this.gravity = a.getInteger(C0000R.styleable.CoordinatorLayout_LayoutParams_android_layout_gravity, CoordinatorLayout.TYPE_ON_INTERCEPT);
            this.mAnchorId = a.getResourceId(C0000R.styleable.CoordinatorLayout_LayoutParams_layout_anchor, -1);
            this.anchorGravity = a.getInteger(C0000R.styleable.CoordinatorLayout_LayoutParams_layout_anchorGravity, CoordinatorLayout.TYPE_ON_INTERCEPT);
        ...
        boolean isBlockingInteractionBelow(CoordinatorLayout parent, View child) {
            ...
            boolean blocksInteractionBelow = (this.mBehavior != null ? this.mBehavior.blocksInteractionBelow(parent, child) : CoordinatorLayout.TYPE_ON_INTERCEPT) | this.mDidBlockInteraction;
    ...
    protected static class SavedState extends BaseSavedState {
        ...
        static class C02281 implements ParcelableCompatCreatorCallbacks<SavedState> {
            C02281() {
            }
        ...
        public SavedState(Parcel source, ClassLoader loader) {
            ...
            for (int i = CoordinatorLayout.TYPE_ON_INTERCEPT; i < size; i += CoordinatorLayout.TYPE_ON_TOUCH) {
        ...
        static {
            CREATOR = ParcelableCompat.newCreator(new C02281());
        }
    }
    ...
    public CoordinatorLayout(Context context, AttributeSet attrs) {
        this(context, attrs, TYPE_ON_INTERCEPT);
    }

    public CoordinatorLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        this.mLayoutDependencyComparator = new C00021();
        ...
        int keylineArrayRes = a.getResourceId(C0000R.styleable.CoordinatorLayout_keylines, TYPE_ON_INTERCEPT);
        if (keylineArrayRes != 0) {
            Resources res = context.getResources();
            this.mKeylines = res.getIntArray(keylineArrayRes);
            float density = res.getDisplayMetrics().density;
            int count = this.mKeylines.length;
            for (int i = TYPE_ON_INTERCEPT; i < count; i += TYPE_ON_TOUCH) {
    ...
    private void resetTouchBehaviors() {
        if (this.mBehaviorTouchView != null) {
            ...
            if (b != null) {
                ...
                MotionEvent cancelEvent = MotionEvent.obtain(now, now, 3, 0.0f, 0.0f, TYPE_ON_INTERCEPT);
        ...
        for (int i = TYPE_ON_INTERCEPT; i < childCount; i += TYPE_ON_TOUCH) {
            ...
            if ((intercepted || newBlock) && action != 0) {
                if (b != null) {
                    if (cancelEvent == null) {
                        long now = SystemClock.uptimeMillis();
                        cancelEvent = MotionEvent.obtain(now, now, 3, 0.0f, 0.0f, TYPE_ON_INTERCEPT);
                    }
                    switch (type) {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (action == TYPE_ON_TOUCH || action == 3) {
            resetTouchBehaviors();
        }
        return intercepted;
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        boolean isRtl = layoutDirection == TYPE_ON_TOUCH;
    ...
    private void dispatchChildApplyWindowInsets(WindowInsetsCompat insets) {
        if (!insets.isConsumed()) {
            int z = getChildCount();
            for (int i = TYPE_ON_INTERCEPT; i < z; i += TYPE_ON_TOUCH) {
                View child = getChildAt(i);
    ...
    public void onDraw(Canvas c) {
        ...
        if (this.mDrawStatusBarBackground && this.mStatusBarBackground != null) {
            int inset = this.mLastInsets != null ? this.mLastInsets.getSystemWindowInsetTop() : TYPE_ON_INTERCEPT;
            if (inset > 0) {
                this.mStatusBarBackground.setBounds(TYPE_ON_INTERCEPT, TYPE_ON_INTERCEPT, getWidth(), inset);
    ...
    void getChildRect(View child, boolean transform, Rect out) {
        if (child.isLayoutRequested() || child.getVisibility() == 8) {
            out.set(TYPE_ON_INTERCEPT, TYPE_ON_INTERCEPT, TYPE_ON_INTERCEPT, TYPE_ON_INTERCEPT);
        } else if (transform) {
    ...
    private void layoutChildWithKeyline(View child, int keyline, int layoutDirection) {
        ...
        switch (vgrav) {
            case ItemTouchHelper.START /*16*/:
                top = TYPE_ON_INTERCEPT + (childHeight / 2);
                break;
            case C0220R.styleable.Theme_panelMenuListTheme /*80*/:
                top = TYPE_ON_INTERCEPT + childHeight;
                break;
        }
        left = Math.max(getPaddingLeft() + lp.leftMargin, Math.min(left, ((width - getPaddingRight()) - childWidth) - lp.rightMargin));
        top = Math.max(getPaddingTop() + lp.topMargin, Math.min(top, ((height - getPaddingBottom()) - childHeight) - lp.bottomMargin));
    ...
    void dispatchOnDependentViewChanged(boolean fromNestedScroll) {
        ...
        for (int i = TYPE_ON_INTERCEPT; i < childCount; i += TYPE_ON_TOUCH) {
            ...
            View child = (View) this.mDependencySortedChildren.get(i);
            LayoutParams lp = (LayoutParams) child.getLayoutParams();
            for (j = TYPE_ON_INTERCEPT; j < i; j += TYPE_ON_TOUCH) {
            ...
            if (!oldRect.equals(newRect)) {
                ...
                for (j = i + TYPE_ON_TOUCH; j < childCount; j += TYPE_ON_TOUCH) {
        ...
        for (int i = TYPE_ON_INTERCEPT; i < childCount; i += TYPE_ON_TOUCH) {
            ...
            if (lp.isNestedScrollAccepted()) {
                ...
                if (viewBehavior != null) {
                    ...
                    iArr[TYPE_ON_INTERCEPT] = TYPE_ON_INTERCEPT;
                    viewBehavior.onNestedPreScroll(this, view, target, dx, dy, this.mTempIntPair);
                    xConsumed = dx > 0 ? Math.max(xConsumed, this.mTempIntPair[TYPE_ON_INTERCEPT]) : Math.min(xConsumed, this.mTempIntPair[TYPE_ON_INTERCEPT]);
                    yConsumed = dy > 0 ? Math.max(yConsumed, this.mTempIntPair[TYPE_ON_TOUCH]) : Math.min(yConsumed, this.mTempIntPair[TYPE_ON_TOUCH]);
                    accepted = true;
                }
            }
        }
        consumed[TYPE_ON_INTERCEPT] = xConsumed;
        consumed[TYPE_ON_TOUCH] = yConsumed;
        if (accepted) {
            dispatchOnDependentViewChanged(true);
        }
    }
    ...
    private static void selectionSort(List<View> list, Comparator<View> comparator) {
        if (list != null && list.size() >= 2) {
            ...
            for (i = TYPE_ON_INTERCEPT; i < count; i += TYPE_ON_TOUCH) {
                int min = i;
                for (int j = i + TYPE_ON_TOUCH; j < count; j += TYPE_ON_TOUCH) {
