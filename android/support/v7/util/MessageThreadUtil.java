...
class MessageThreadUtil<T> implements ThreadUtil<T> {
    ...
    class C02821 implements MainThreadCallback<T> {
        ...
        class C01551 implements Runnable {
            ...
            public void run() {
                SyncQueueItem msg = C02821.this.mQueue.next();
                while (msg != null) {
                    switch (msg.what) {
                        case C02821.UPDATE_ITEM_COUNT /*1*/:
                            C02821.this.val$callback.updateItemCount(msg.arg1, msg.arg2);
                            break;
                        case C02821.ADD_TILE /*2*/:
                            C02821.this.val$callback.addTile(msg.arg1, (Tile) msg.data);
                            break;
                        case C02821.REMOVE_TILE /*3*/:
                            C02821.this.val$callback.removeTile(msg.arg1, msg.arg2);
                            break;
                        default:
                            Log.e("ThreadUtil", "Unsupported message, what=" + msg.what);
                            break;
                    }
                    msg = C02821.this.mQueue.next();
                }
            }
        }

        C02821(MainThreadCallback mainThreadCallback) {
            this.val$callback = mainThreadCallback;
            this.mQueue = new MessageQueue();
            this.mMainThreadHandler = new Handler(Looper.getMainLooper());
            this.mMainThreadRunnable = new C01551();
        }
    ...
    class C02832 implements BackgroundCallback<T> {
        ...
        class C01561 implements Runnable {
            ...
            public void run() {
                while (true) {
                    SyncQueueItem msg = C02832.this.mQueue.next();
                    if (msg != null) {
                        switch (msg.what) {
                            case C02832.REFRESH /*1*/:
                                C02832.this.mQueue.removeMessages(C02832.REFRESH);
                                C02832.this.val$callback.refresh(msg.arg1);
                                break;
                            case C02832.UPDATE_RANGE /*2*/:
                                C02832.this.mQueue.removeMessages(C02832.UPDATE_RANGE);
                                C02832.this.mQueue.removeMessages(C02832.LOAD_TILE);
                                C02832.this.val$callback.updateRange(msg.arg1, msg.arg2, msg.arg3, msg.arg4, msg.arg5);
                                break;
                            case C02832.LOAD_TILE /*3*/:
                                C02832.this.val$callback.loadTile(msg.arg1, msg.arg2);
                                break;
                            case C02832.RECYCLE_TILE /*4*/:
                                C02832.this.val$callback.recycleTile((Tile) msg.data);
                                break;
                            default:
                                Log.e("ThreadUtil", "Unsupported message, what=" + msg.what);
                                break;
                    ...
                    C02832.this.mBackgroundRunning.set(false);
                    return;
                }
            }
        }

        C02832(BackgroundCallback backgroundCallback) {
            ...
            this.mBackgroundRunnable = new C01561();
        }
