...
public class AsyncListUtil<T> {
    ...
    public static abstract class ViewCallback {
        ...
        public void extendRangeInto(int[] range, int[] outRange, int scrollHint) {
            int i;
            int fullRange = (range[HINT_SCROLL_DESC] - range[0]) + HINT_SCROLL_DESC;
            int halfRange = fullRange / HINT_SCROLL_ASC;
            int i2 = range[0];
            if (scrollHint == HINT_SCROLL_DESC) {
                i = fullRange;
            } else {
                i = halfRange;
            }
            outRange[0] = i2 - i;
            i = range[HINT_SCROLL_DESC];
            if (scrollHint != HINT_SCROLL_ASC) {
                fullRange = halfRange;
            }
            outRange[HINT_SCROLL_DESC] = i + fullRange;
        }
    }

    /* renamed from: android.support.v7.util.AsyncListUtil.1 */
    class C02801 implements MainThreadCallback<T> {
        C02801() {
        }
        ...
        private boolean isRequestedGeneration(int generation) {
            return generation == AsyncListUtil.this.mRequestedGeneration ? true : AsyncListUtil.DEBUG;
        }
    }

    /* renamed from: android.support.v7.util.AsyncListUtil.2 */
    class C02812 implements BackgroundCallback<T> {
        ...
        public void updateRange(int rangeStart, int rangeEnd, int extRangeStart, int extRangeEnd, int scrollHint) {
            if (rangeStart <= rangeEnd) {
                ...
                if (scrollHint == 1) {
                    ...
                    requestTiles(AsyncListUtil.this.mTileSize + lastVisibleTileStart, this.mLastRequiredTileStart, scrollHint, AsyncListUtil.DEBUG);
                    return;
                }
                requestTiles(firstVisibleTileStart, this.mLastRequiredTileStart, scrollHint, AsyncListUtil.DEBUG);
    ...
    public AsyncListUtil(Class<T> klass, int tileSize, DataCallback<T> dataCallback, ViewCallback viewCallback) {
        ...
        this.mMainThreadCallback = new C02801();
        this.mBackgroundCallback = new C02812();
        this.mTClass = klass;
